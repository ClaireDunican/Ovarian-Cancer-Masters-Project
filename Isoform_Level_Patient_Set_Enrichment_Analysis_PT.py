		...
		if name.endswith(".csv"):  #only the patient data files
			if name != "Percentage_Tumour.csv":
				...
				for line in patient_variants:
					...
					patient_gene_variants.append("G: " + line[0] + ", V: " + line[2]) #get the gene name
...
with open("PT_isoforms_ordered.csv", "a", newline='') as my_file:
...
#np.savetxt("gsea_data.txt", patient_counts, delimiter=",", newline = "\n", fmt="%s") #save to a file
#patient_counts = np.delete(patient_counts, 0, axis=1)
#patient_counts = np.delete(patient_counts, 0, axis=1)
#np.savetxt("gsea_data.txt", patient_counts, delimiter=" ", newline = "\n", fmt="%s") #save to a file
...
def enrichment_score(gene_list, correl_vector, gene_set, weighted_score_type=0, 
                     nperm=10000, rs=np.random.RandomState(), single=False, scale=False): #nperm equal to permutation number: 1000?
...
enrichment_scores, nEnrichmentScores, enrichmentPVals, fdrs = gsea_significance(es_values, es_null_values)  #7 isoforms at fdr, 963 before
...
fdrs.insert(0, "fdrs")
qv.insert(0, "qv")
my_results = np.column_stack((genes, corresponding_genes, enrichment_scores, nEnrichmentScores, enrichmentPVals, qv))
my_results = np.matrix(my_results)
np.savetxt("Enrichment_Analysis_Isoform_Results_TP.csv", my_results, delimiter=",", newline = "\n", fmt="%s")
...
with open('isoform_gene_sets.gmt') as genesets:
    ...
    with open('Isoform_Present_In_All_Samples_TP.csv', 'w') as filey:
    ...
    with open('Isoform_Present_In_Cancer_Samples_TP.csv', 'w') as filey:
...
header = ['Isoform', 'Gene', 'Enrichment_Score','normalised_Enrichemnt_Score','p_value', 'q_value']
with open("Significant_isoforms_TP.csv", "a", newline='') as my_file:
	wr = csv.writer(my_file, dialect='excel')
	wr.writerow(header)
