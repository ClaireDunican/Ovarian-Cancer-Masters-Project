#Gene Set Enrichment Analysis at the isoform level, in python3
		...
		if name.endswith(".csv"):  #only the patient data files
			if name != "Disease_Score.csv":
				...
				for line in patient_variants:
					...
					patient_gene_variants.append("G: " + line[0] + ", V: " + line[2]) #line[2]) #get the gene name #here: "G: " + line[0] + ", V: " + line[2]
...
overall_unique_variants = sorted(list(set(overall_unique_variants)))
...
for gene in overall_unique_variants:
	...
	gene = str(gene)
	corresponding_genes.append(gene) #8161
overall_unique_variants = just_ID

sets = np.column_stack((corresponding_genes, just_ID))
	...
	for name in files: 
		...
		if name.endswith(".csv"):  #only the patient data files
			if name != "Disease_Score.csv":
				...
				for line in patient_variants:
					line = line.split(',') #split each line by a comma
					patient_gene_variants.append(line[2]) #get the isoform name
				patient_gene_variants = sorted(list(set(patient_gene_variants))) #converting to a set removes the non-unique genes then convert back to a list and sort alphabeticall
...
with open("DS_isoforms_ordered.csv", "a", newline='') as my_file:
...
with open('isoform_gene_sets.gmt', 'w') as f:
	for line in gene_Set_matrix:
		line = list(np.array(line).reshape(-1,))
		line = '	'.join(line)
		f.write(line + "\n")
with open('isoform_gene_sets.gmt') as gmt:
    gmt.read()
...
results = gsea_gmt_parser(gmt='isoform_gene_sets.gmt', min_size = 2, max_size = 28, gene_list=None) #only includes those with at least two at at most 28 patients as those isoforms in a single patient are not informative nor are those in all the patients

def enrichment_score(gene_list, correl_vector, gene_set, weighted_score_type=0, 
                     nperm=1000, rs=np.random.RandomState(), single=False, scale=False): #nperm equal to permutation number: 1000?
...
for k, v in results.items(): #510 genes are 1 < patients < 29
    my_dictt = {k : list(v)}
    isoformys.append(str(k))
    for gene_var in sets:
        gene = list(np.array(gene_var).reshape(-1,)) 
        if k == str(gene[1]):
            corresponding_genes.append(str(gene[0]))
    es, esnull, hit_ind, RES = enrichment_score(gene_list=gene_list, correl_vector=correlation_vector, gene_set=my_dictt, weighted_score_type=1, nperm=20000, rs=np.random.RandomState(), single=False, scale=False)
    es_values.append(es)
...
def gsea_significance(enrichment_scores, enrichment_nulls):
    ...
    return enrichment_scores, enrichmentPVals #nEnrichmentScores,

enrichment_scores, enrichmentPVals = gsea_significance(es_values, es_null_values)  #7 isoforms at fdr, 963 before  nEnrichmentScores,

def estimate(pv, m=None, verbose=False, lowmem=False, pi0=None): #from: 
    """
...
for qv_val in qv:
    if qv_val < 0.05:
        count += 1
print("The number of statistically significant isoforms after multiple testing correction is: ", count)

corresponding_genes.insert(0, "Gene")
qv = qv.tolist()
genes = isoformys
genes.insert(0, "Isoform")
...
qv.insert(0, "qv")
my_results = np.column_stack((genes, corresponding_genes, enrichment_scores, enrichmentPVals, qv)) #nEnrichmentScores, enrichmentPVals, qv))
my_results = np.matrix(my_results)
np.savetxt("Enrichment_Analysis_Isoform_Results_DS.csv", my_results, delimiter=",", newline = "\n", fmt="%s")

#Get the genes in every patient i.e. those that are likely cancer associated
with open('isoform_gene_sets.gmt') as genesets:
    ...
    with open('Isoform_Present_In_All_Samples_DS.csv', 'w') as filey:
        filey.write("Isoform_In_All_Samples" + "\n")
    ...
    with open('Isoform_Present_In_Cancer_Samples_DS.csv', 'w') as filey:
        filey.write("Isoform_In_All_Cancer_Samples" + "\n")
...
header = ['Isoform', 'Gene', 'Enrichment_Score','p_value', 'q_value']
with open("Significant_isoforms_DS.csv", "a", newline='') as my_file:
	wr = csv.writer(my_file, dialect='excel')
	wr.writerow(header)
...
genes_numbers = Counter(rows)

import matplotlib.pyplot as plt
bars = genes_numbers.keys()
print(bars)
height = genes_numbers.values()
print(height)
