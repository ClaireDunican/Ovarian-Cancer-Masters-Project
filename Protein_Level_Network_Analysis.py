#Pairwise Comparisons #top 10% maybe plot??
	...
	for name in files: 
		...
		if name.endswith(".csv"):  #only the patient data files
			if name != "Matrix_index_values.csv":
				...
				#patient_gene_variants.append(patient)
...
np.savetxt("Genes_For_Pairwise_Comparisons.csv", patient_counts, delimiter=",", newline = "\n", fmt="%s") #save to a file
...
totals_per_patient = np.sum(patient_counts_2, axis=1)
...
np.savetxt("Similarity_Statistics_Gene_Level.csv", simularity_stuff, delimiter=",", newline = "\n", fmt="%s") #save to a file
relevant_rows = []
compliments = []
...
np.savetxt("Similarity_Statistics_Gene_Level_top_10_percent.csv", relevant_rows, delimiter=",", newline = "\n", fmt="%s") #save to a file
...
#control = ['G94', 'G93', 'G88', 'G57', 'G72', 'G58', 'G42', 'G10']
#pre_chemotherapy = ['G82', 'G76', 'G69', 'G54', 'G43', 'G36', 'G30', 'G138', 'G124']
#good_resp = ['G92', 'G85', 'G81', 'G67', 'G29a', 'G102']
#bad_resp = ['G79', 'G75',  'G33a', 'G17', 'G15', 'G11']
...
for row in relevant_rows:
	...
	G.add_edge(node_1, node_2, weight=int(edgey))
...
nx.draw(G, node_color=carac['myvalue'].cat.codes, cmap=plt.cm.Set1, with_labels=True, font_size = 9, node_size=700)
plt.savefig("all_subsets_gene.png") #save as png
plt.show() #display

print("The attribute assortivity coefficient for all subsets: ", nx.attribute_assortativity_coefficient(G,'type'))
...
plt.savefig("cancer_vs_control_gene.png") #save as png
plt.show() #display

print("The attribute assortivity coefficient for control vs. cancer: ", nx.attribute_assortativity_coefficient(F,'type'))
...
plt.savefig("Pre-chemo_vs_everything_else_gene.png") #save as png
plt.show() #display
...
plt.yticks(range(0, 275, 10))
plt.title("A. Proteins")
...
titles = ["Genes", "Frequency"]
everything = np.insert(everything, 0, titles, axis = 0)
np.savetxt("Total_Gene_Counts.csv", everything, delimiter=",", newline = "\n", fmt="%s")
gene_list = []

for suby in everything:
    suby = list(np.array(suby).reshape(-1,))
    gene_list.append(suby[0])

import matplotlib.pyplot as plt
import pandas as pd
df=pd.read_csv('Total_Gene_Counts.csv')
days_values=df.set_index("Genes").loc[gene_list].plot(kind="bar", legend=False)
plt.xlabel('Genes')
plt.ylabel('Frequency')
plt.title('The total count of each gene in the patient subset')
